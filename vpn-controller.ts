    ...
    private startWatchdog(): void {
        ...
        this.watchdog = setInterval(() => {
            ...
            if (owner && owner.pid !== process.pid && !processAlive(owner.pid)) {
                void this.stopInternal(false, true).then(() => {
                    this.options.log("info", "watchdog derrubou o WireSock porque o Discord dono (PID " + owner.pid + ") morreu");
                }).catch(error => {
            ...
            if (this.discordPid !== null && this.discordPid !== process.pid && !processAlive(this.discordPid)) {
                void this.stopInternal(false, true).then(() => {
                    this.options.log("info", "watchdog derrubou o WireSock porque o Discord dono (PID discord=" + this.discordPid + ") morreu");
                }).catch(error => {
